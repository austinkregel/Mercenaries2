...
-- 1. Spawn the unit at the player's position
local ok, h = pcall(spawn_fn, "VZ", "DoubleSpawnedVZ", { px, py, pz }, nil, yaw)
if ok and h then
    log(string.format("SpawnActor success! Handle=%s", tostring(h)))
    
    -- 2. Teleport both to the same local area
    -- Spawned unit goes 3 meters offset
    local tx, ty, tz = px + 3, py + 0.5, pz + 3
    log(string.format("Teleporting spawned unit to: %.2f, %.2f, %.2f", tx, ty, tz))
    pcall(Object.SetPosition, h, tx, ty, tz)
    
    -- Player goes to original position
    log(string.format("Teleporting player to: %.2f, %.2f, %.2f", px, py, pz))
