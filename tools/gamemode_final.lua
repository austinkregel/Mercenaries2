...
wd.Start = function()
  ...
  local pmc = _MODULES.vz.MrxFactionManager._tFactions.Pmc.uGuid
  local gur = _MODULES.vz.MrxFactionManager._tFactions.Gur.uGuid
  h.MessageBox:AddMessage({vPlayer=p.GetLocalPlayer(),sMessage="WAVE "..wd.v.." START!",nPriority=1,nDuration=4})
  pcall(a.SetAttitude, pmc, gur, 1)
  pcall(a.SetAttitude, gur, pmc, 1)
  pcall(a.SetRelation, pmc, gur, -100)
  pcall(a.SetRelation, gur, pmc, -100)
  local nh = math.ceil(wd.v/2)
  local count = 0
  for w=1,nh do
    local sx, sz = px+35+(w*4), pz+35-(w*4)
    local ok, hl = pcall(Pg.Spawn, "UH1 Transport (GR) (Full)", sx, py+40, sz, yaw, false, true)
    if ok and hl then
  ...
  h.MessageBox:AddMessage({vPlayer=p.GetLocalPlayer(),sMessage=count.." GUERRILLAS SEEKING YOU!",nPriority=1,nDuration=3})
...
wd.Stop = function()
  ...
  local pmc = _MODULES.vz.MrxFactionManager._tFactions.Pmc.uGuid
  local gur = _MODULES.vz.MrxFactionManager._tFactions.Gur.uGuid
  pcall(a.SetAttitude, pmc, gur, 3)
  pcall(a.SetAttitude, gur, pmc, 3)
  pcall(a.SetRelation, pmc, gur, 100)
  pcall(a.SetRelation, gur, pmc, 100)
  for _, enemy in ipairs(wd.e) do
