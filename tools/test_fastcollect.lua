...
for i, h in ipairs(humans) do
    ...
    log(string.format("  [%d] Handle=%s, Valid=%s, Alive=%s, Name=%s, LocalizedName=%s, Distance=%.2fm",
        i, tostring(h), tostring(valid), tostring(alive), tostring(name), tostring(locName), dist))
end
